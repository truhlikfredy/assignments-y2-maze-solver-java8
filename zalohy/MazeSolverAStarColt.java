...
public class MazeSolverAStarColt implements MazeSolver {
	...
	private SparseObjectMatrix2D	visit;
	private SparseObjectMatrix2D	visitedAlready;
	 ...
	 */
	public MazeSolverAStarColt(Maze maze) throws Exception {
		...
		this.visit = new SparseObjectMatrix2D(maze.getWidth(), maze.getHeight());
		this.visitedAlready = new SparseObjectMatrix2D(maze.getWidth(), maze.getHeight());
	...
	public void addStartPosition(Point origin) throws Exception {
		visit.setQuick(origin.x,origin.y, new MazeNode(null, 0, origin, destinations));
		this.origin = origin;
	}
	...
	public List<Point> backTracePath() {
		...
		Optional<Point> destination = destinations.stream().filter(point -> visitedAlready.getQuick(point.x, point.y)!=null)
				.findFirst();
		...
		while (currentStep != null) {
			...
			currentStep = (Point)(visitedAlready.getQuick(currentStep.x,currentStep.y));
			iteration++;
		}
